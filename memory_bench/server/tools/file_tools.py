...
class FileTools:
    ...
    def _safe_path(self, path_str: str, write_mode: bool = False) -> Path:
        ...
        if write_mode:
            ...
            if not resolved_str.startswith(str(self.memory_bench)):
                raise SecurityError(f"写入操作超出 memory_bench 范围：{full_path}\n允许范围：{self.memory_bench}")
        else:
            # 读取模式：允许整个 workspace
            if not resolved_str.startswith(str(self.workspace)):
                raise SecurityError(f"读取操作超出 workspace 范围：{full_path}\n允许范围：{self.workspace}")
    ...
    def read(self, path: str | None = None, purpose: str | None = None) -> FileOperationResult:
        ...
        Examples:
            # 读取特定文件
            tools.read("memory_bench/server/prompts/emotion/base_persona.txt")
        ...
        try:
            ...
            if full_path.is_dir():
                ...
                return FileOperationResult(success=True, path=str(full_path), content="\n".join(files))

            content = full_path.read_text(encoding="utf-8")
            return FileOperationResult(success=True, path=str(full_path), content=content)

        except SecurityError as e:
            return FileOperationResult(success=False, path=path or "", error=str(e))
    ...
    def write(
        self, content: str, path: str | None = None, purpose: str | None = None, append: bool = False
    ) -> FileOperationResult:
        """
        写入文件
        ...
        try:
            ...
            return FileOperationResult(success=True, path=str(full_path), content=None)

        except SecurityError as e:
            return FileOperationResult(success=False, path=path or "", error=str(e))
        except Exception as e:
            return FileOperationResult(success=False, path=path or "", error=f"写入失败：{e}")
    ...
    def edit(self, path: str, old_text: str, new_text: str) -> FileOperationResult:
        ...
        try:
            ...
            if old_text not in content:
                return FileOperationResult(
                    success=False, path=str(full_path), error="未找到要替换的原文（必须精确匹配）"
                )
            ...
            return FileOperationResult(success=False, path=path, error=str(e))
        except Exception as e:
            return FileOperationResult(success=False, path=path, error=f"编辑失败：{e}")
