#!/usr/bin/env python3
"""将 memory snapshot JSONL 转为图谱节点/边（V0 metadata 归属图）。
...
DEFAULT_OUT_DIR = Path("memory_bench/logs/graphify/meta")
DEFAULT_STATE_DB = Path("memory_bench/state/graphify/meta.sqlite")
...
def build_parser() -> argparse.ArgumentParser:
    """构建 graph_ir_export_meta 的命令行参数解析器。
    ...
    parser = argparse.ArgumentParser(
        description="Graphify memory snapshot JSONL into graph nodes/edges (V0 metadata ownership graph)",
    ...
    for cmd in ("add", "dry-run"):
        ...
        cmd_parser.add_argument("--prefix", type=str, default="meta")
