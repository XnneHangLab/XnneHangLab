...
class AzureASRConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(en="API key for Azure ASR service", zh="Azure ASR 服务的 API 密钥"),
        "region": Description(en="Azure region (e.g., eastus)", zh="Azure 区域（如 eastus)"),
...
class FasterWhisperConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "model_path": Description(en="Path to the Faster Whisper model", zh="Faster Whisper 模型路径"),
        "download_root": Description(en="Root directory for downloading models", zh="模型下载根目录"),
...
class WhisperCPPConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "model_name": Description(en="Name of the Whisper model", zh="Whisper 模型名称"),
        "model_dir": Description(en="Directory containing Whisper models", zh="Whisper 模型目录"),
        "print_realtime": Description(en="Print output in real-time", zh="实时打印输出"),
        "print_progress": Description(en="Print progress information", zh="打印进度信息"),
        "language": Description(en="Language code (en, zh, or auto)", zh="语言代码（en、zh 或 auto）"),
    }
...
class WhisperConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "download_root": Description(en="Root directory for downloading models", zh="模型下载根目录"),
        "device": Description(en="Device to use for inference (cpu or cuda)", zh="推理设备（cpu 或 cuda）"),
    }
...
class FunASRConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "vad_model": Description(en="Voice Activity Detection model", zh="语音活动检测模型"),
        ...
        "use_itn": Description(en="Enable inverse text normalization", zh="启用反向文本归一化"),
        "language": Description(en="Language code (zh, en, or auto)", zh="语言代码（zh、en 或 auto）"),
    }
...
class GroqWhisperASRConfig(I18nMixin):
    ...
    api_key: str = Field(..., alias="api_key")
    model: Literal["whisper-large-v3-turbo", "whisper-large-v3"] = Field("whisper-large-v3-turbo", alias="model")
    lang: Optional[str] = Field(None, alias="lang")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(en="API key for Groq Whisper ASR", zh="Groq Whisper ASR 的 API 密钥"),
        "model": Description(
...
class SherpaOnnxASRConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "model_type": Description(en="Type of ASR model to use", zh="要使用的 ASR 模型类型"),
        "encoder": Description(
        ...
        "paraformer": Description(en="Path to paraformer model", zh="Paraformer 模型路径"),
        ...
        "whisper_encoder": Description(en="Path to Whisper encoder model", zh="Whisper 编码器模型路径"),
        "whisper_decoder": Description(en="Path to Whisper decoder model", zh="Whisper 解码器模型路径"),
        "sense_voice": Description(en="Path to SenseVoice model", zh="SenseVoice 模型路径"),
    ...
    def check_model_paths(cls, values: "SherpaOnnxASRConfig", info: ValidationInfo):
        ...
        if model_type == "transducer":
            if not all([values.encoder, values.decoder, values.joiner, values.tokens]):
                raise ValueError("encoder, decoder, joiner, and tokens must be provided for transducer model type")
        elif model_type == "paraformer":
            if not all([values.paraformer, values.tokens]):
                raise ValueError("paraformer and tokens must be provided for paraformer model type")
        elif model_type == "nemo_ctc":
            if not all([values.nemo_ctc, values.tokens]):
                raise ValueError("nemo_ctc and tokens must be provided for nemo_ctc model type")
        elif model_type == "wenet_ctc":
            if not all([values.wenet_ctc, values.tokens]):
                raise ValueError("wenet_ctc and tokens must be provided for wenet_ctc model type")
        elif model_type == "tdnn_ctc":
            if not all([values.tdnn_model, values.tokens]):
                raise ValueError("tdnn_model and tokens must be provided for tdnn_ctc model type")
        elif model_type == "whisper":
            if not all([values.whisper_encoder, values.whisper_decoder, values.tokens]):
                raise ValueError("whisper_encoder, whisper_decoder, and tokens must be provided for whisper model type")
        elif model_type == "sense_voice":
            if not all([values.sense_voice, values.tokens]):
                raise ValueError("sense_voice and tokens must be provided for sense_voice model type")
...
class ASRConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "asr_model": Description(en="Speech-to-text model to use", zh="要使用的语音识别模型"),
        "azure_asr": Description(en="Configuration for Azure ASR", zh="Azure ASR 配置"),
        "faster_whisper": Description(en="Configuration for Faster Whisper", zh="Faster Whisper 配置"),
        "whisper_cpp": Description(en="Configuration for WhisperCPP", zh="WhisperCPP 配置"),
        ...
        "groq_whisper_asr": Description(en="Configuration for Groq Whisper ASR", zh="Groq Whisper ASR 配置"),
        "sherpa_onnx_asr": Description(en="Configuration for Sherpa Onnx ASR", zh="Sherpa Onnx ASR 配置"),
    }
