...
class ServiceContext:
    ...
    async def reload_runtime_from_current_settings(self) -> None:
        ...
        # 先 close 旧 agent（会触发所有 HookPlugin.stop()，释放单例计数等资源），
        # 再创建新实例，避免单例插件（如 MoodChatPlugin）在新实例创建时误报冲突。
        ...
        try:
            ...
            logger.error("Reload failed — agent is now unavailable until next successful reload.")
            raise
