...
mem0-ingest:
  uv run python memory_bench/scripts/replay_mem0.py ingest --force
...
claims-extract:
  ...
  uv run python memory_bench/scripts/claimify_all.py --input "$latest_export" --workers 2 --force

claims-compile:
  uv run python memory_bench/scripts/compiled_claims.py --force
...
graph-claims-cypher:
  claim_nodes=$(uv run python memory_bench/scripts/latest_file.py --export-dir memory_bench/logs/claims/graphify --glob "claims_nodes_*.jsonl") && \
  claim_edges=$(uv run python memory_bench/scripts/latest_file.py --export-dir memory_bench/logs/claims/graphify --glob "claims_edges_*.jsonl") && \
  uv run python memory_bench/scripts/neo4j_cypher_export.py \
    --nodes "$claim_nodes" \
    --edges "$claim_edges" \
