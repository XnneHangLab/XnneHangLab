...
- **描述**：按 [`.github/PULL_REQUEST_TEMPLATE.md`](https://github.com/XnneHangLab/XnneHangLab/blob/dev/.github/PULL_REQUEST_TEMPLATE.md) 格式填写（动机 / 解决方案 / 类型 checkbox）
- **目标分支**：一律向 `dev` 提 PR
- **RoadMap 联动**：PR 合入前，检查本次改动是否覆盖了 [RoadMap](./roadmap) 中的某项。如果是，请在 PR 描述里注明，并在同一个 PR 中将该条目从 `roadmap.md` 移除。
...
6. **PR 描述必须遵循 [`.github/PULL_REQUEST_TEMPLATE.md`](https://github.com/XnneHangLab/XnneHangLab/blob/dev/.github/PULL_REQUEST_TEMPLATE.md)** 的格式（动机 / 解决方案 / 类型 checkbox）
7. **PR 合入前检查 RoadMap**：若本次改动覆盖了 [RoadMap](./roadmap) 中的某项，在 PR 描述里说明，并在同一 PR 中将该条目从 RoadMap 移除
